...
import { ALL_GAME_FISH, top20ForType, successScore, suggestGear, bboxFromCenterRadius, cacheGet, cacheSet, haversineMiles, bestWindows } from './lib.js'
...
function useGeolocation(){
  const [pos,setPos]=useState(null); const [err,setErr]=useState(null);
  const get=()=>{
...
function Map({ center, wb, radiusMi }){
  useEffect(()=>{
    ...
    if(wb){ L.marker([wb.lat, wb.lon]).addTo(map).bindPopup(`${wb.name}`); map.fitBounds(L.latLngBounds([[center.lat,center.lon],[wb.lat,wb.lon]]) , { padding:[30,30]}); }
    return ()=>map.remove();
...
async function fetchUSGSSites(center, radiusMi){
  ...
  const key = `sites:${params.toString()}`; const cached = cacheGet(key); if(cached) return cached;
  const r = await fetch(url); const j = await r.json();
  const list = (j?.value?.site || []).map(s=>({ id: s.siteCode?.[0]?.value, name: s.siteName, lat: parseFloat(s.geoLocation?.geogLocation?.latitude), lon: parseFloat(s.geoLocation?.geogLocation?.longitude), type: (s.siteType?.[0]?.value||'').match(/Lake|Reservoir/i)?'Lake':((s.siteType?.[0]?.value||'').match(/Stream|River/i)?'River':'Water') })).filter(s=>Number.isFinite(s.lat)&&Number.isFinite(s.lon));
...
export default function App(){
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <footer className="text-xs text-slate-500 text-center py-6">v1.0 • Data: Open‑Meteo, USGS, OpenStreetMap. Some values may be estimated when hydrology is unavailable.</footer>
    </div>
  )
}
