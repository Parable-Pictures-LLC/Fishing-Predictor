...
function Map({ center, wb, radiusMi }) {
  ...
  return <div id="map" className="w-full h-80 bg-slate-800" aria-label="Map" />;
}
...
async function fetchUSGSSites(center, radiusMi) {
  ...
  if (list.length === 0) {
    ...
    const query = `[out:json][timeout:25];
      ...
      out center;`;
    const resp = await fetch(OVERPASS_URL, { method:'POST', headers:{'Content-Type':'text/plain'}, body: query });
    const jj = await resp.json();
    list = (jj.elements || []).map(e => ({
      id: String(e.id),
...
async function fetchUSGSConditions(siteId){
  ...
  const key = `iv:${siteId}`; const cached = cacheGet(key); if(cached) return cached;
...
async function fetchWeather(lat,lon,dateISO){
  const d = new Date(dateISO); const y=d.getUTCFullYear(), m=d.getUTCMonth()+1, day=d.getUTCDate();
  const params = new URLSearchParams({
  ...
  const key = `wx:${lat.toFixed(3)},${lon.toFixed(3)}:${dateISO}`; const cached = cacheGet(key); if(cached) return cached;
  const r = await fetch(url); const j = await r.json();
...
async function fetchPOIs(lat,lon,radiusMi, types=['boat_ramp','shop_fishing']){
  ...
  const j = await r.json();
  const items = (j.elements||[]).map(e=>({ id:e.id, lat:e.lat, lon:e.lon, name:e.tags?.name||e.tags?.brand||'(Unnamed)', type: e.tags?.amenity||e.tags?.shop }));
...
export default function App(){
  ...
  const selectedWaterType = useMemo(()=> siteInfo?.type || 'Water', [siteInfo]);

  useEffect(()=>{ if(pos && !center){ setCenter(pos); setCenterText(`${pos.lat.toFixed(5)},${pos.lon.toFixed(5)}`); } },[pos]);
  ...
  useEffect(()=>{
    ...
    const s = sites.find(x=>x.id===selectedSiteId); if(s) setSiteInfo(s);
  ...
  const score = useMemo(()=> (derived && siteInfo)? successScore(species, selectedWaterType, derived) : null, [derived, species, selectedWaterType]);
  const gear = useMemo(()=> (derived && siteInfo)? suggestGear(species, selectedWaterType, derived) : null, [derived, species, selectedWaterType]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-slate-300 text-sm">Live weather (Open-Meteo), hydrology (USGS), and simple fish-behavior rules.</p>
      </header>
      ...
      <section className="grid md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900 p-4 rounded-xl space-y-3">
          ...
          <p className="text-xs text-slate-400">USGS sites near you; falls back to OSM waterbodies when none exist.</p>
          <select className="w-full bg-slate-800 rounded px-3 py-2" value={selectedSiteId} onChange={e=>setSelectedSiteId(e.target.value)} aria-label="Water body selector">
            {sites.map(s=> (
              <option key={s.id} value={s.id}>{s.name} · {s.type} · {center?haversineMiles(center,{lat:s.lat,lon:s.lon}).toFixed(1):'?'} mi {s.source==='OSM'?'(OSM)':''}</option>
            ))}
          </select>
          {siteInfo && (
        ...
        <div className="bg-slate-900 p-4 rounded-xl space-y-3">
          ...
          <select className="w-full bg-slate-800 rounded px-3 py-2" value={species} onChange={e=>setSpecies(e.target.value)} aria-label="Species selector">
            {(siteInfo? top20ForType(selectedWaterType): ALL_GAME_FISH).map(s=> <option key={s} value={s}>{s}</option>)}
          </select>
